...
{{-- - Author: Gus Kazem --}}
{{-- - https://Gusgraph.com --}}
{{-- - File Path: resources/views/emails/admin/template.blade.php --}}
{{-- ===================================================== --}}
<!doctype html>
<html lang="en">
<head>
    ...
    <title>{{ $subjectLine }} | Sirraty</title>
    <style>
        body { margin:0;background:#f7f4ef;color:#17221c;font-family:Arial,Helvetica,sans-serif; }
        .page { width:100%;padding:37px 0;background:linear-gradient(rgba(247,244,239,.19),rgba(247,244,239,.27)),url('https://res.cloudinary.com/duja2smra/image/upload/emails-Sirr4857_s2ppm3.webp') center/cover no-repeat #f7f4ef; }
        ...
        .art { height:73px;background:linear-gradient(117deg,rgba(36,117,83,.93),rgba(30,185,197,.53)),repeating-linear-gradient(137deg,rgba(255,255,255,.23) 0 1px,transparent 1px 19px); }
        .content { padding:31px 27px; }
        h1 { margin:0 0 15px;font-size:27px;line-height:1.17;color:#17221c; }
        p { margin:0 0 19px;font-size:15px;line-height:1.57;color:#435047; }
        a { color:#247553;font-weight:700; }
            ...
            <div class="panel">
                ...
                <div class="content">
                    ...
                    @else
                        @foreach(preg_split('/\r\n|\r|\n/', $body) as $line)
                            @if(trim($line) !== '')
                                <p>{!! preg_replace('~(https?://[^\s<]+)~', '<a href="$1">$1</a>', e($line)) !!}</p>
                            @endif
                        @endforeach
                    @endif
                </div>
